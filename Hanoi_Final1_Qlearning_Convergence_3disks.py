...
early_stopping = True#VARIED #Stop Qlearning once RMSE convergence achieved
...
#                        Qnew[s_dyna][a_dyna] = Q[s_dyna][a_dyna] + alpha*(M[s_dyna][a_dyna] + gamma * Qmax_dyna - Q[s_dyna][a_dyna]) #update Q based on past experience
        ...
        if i%1000==0:
            print('     Counting 1000 episodes: ',i) #VARIED
        trace[i]=Qrmse
...
#If normal Q learning - create 4 scenarios with the vayring alpha, epsilon but the same gamma and 'n=0' experience replay
    ...
    dimension = 10; alpha, gamma, epsilon = create_paramater_mesh(0.1, 1, dimension, 0.9, 0.9, 1, 0.1, 1, dimension) #VARIED #alpha_from, alpha_to, gamma_from, gamma_to, epsilon_from, epsilon_to, n
...
episodes = 5000 #max number of episodes before stopping
...
max_steps = 10000#VARIED #Limit on number of steps per run (0 to ignore and just cap on episodes)
...
surface_graph=True #VARIED #Note graphs assume multiple alpha/epsilon parameters run to convergence
if surface_graph: 
    ceiling = 10000 
    print()
